...
# --- Header + Status Bar (unified hero block) ---
_parser_status = st.session_state.get("parser_status", "idle")
_vt_status = st.session_state.get("vt_server_status", "unknown")
_vt_configured = st.session_state.get("virustotal_client") is not None
_email_active = st.session_state.get("email_ingestion_module") is not None

# Build status pills HTML
_pills: list[str] = []

if _parser_status == "running":
    _pills.append('<span class="status-pill status-running">⏳ Parser</span>')
elif _parser_status == "error":
    _pills.append('<span class="status-pill status-disconnected">⚠️ Parser</span>')
else:
    _pills.append('<span class="status-pill status-idle">✓ Parser</span>')

if _vt_status == "connected":
    _pills.append('<span class="status-pill status-connected">● VirusTotal</span>')
elif _vt_status in ("timeout", "error"):
    _pills.append('<span class="status-pill status-disconnected">● VirusTotal</span>')
elif _vt_configured:
    _pills.append('<span class="status-pill status-idle">○ VirusTotal</span>')
else:
    _pills.append('<span class="status-pill status-disconnected">○ VT: No Key</span>')

if _email_active:
    _email_status = st.session_state.email_ingestion.get(
        "connection_status", "disconnected"
    )
    if _email_status == "connected":
        _pills.append('<span class="status-pill status-connected">● Email</span>')
    else:
        _pills.append(
            '<span class="status-pill status-disconnected">● Email</span>'
        )

_pills_html = " ".join(_pills)

st.markdown(
    '<div class="roadblock-hero">'
    '<div class="hero-title">🛡️ RoadBlock</div>'
    '<div class="hero-subtitle">Automated Social Honeypot</div>'
    f'<div class="hero-status">{_pills_html}</div>'
    '</div>',
